...
## 工具失败时怎么办\n分割或统计工具返回空、漏检或明显不合理时：**如实写明失败的是哪几张图、可能的原因，并停在那里**。可以建议用户换器官参数、提供像素尺寸或补数据，但**不要自己写阈值分割、纹理分割或任何图像测量代码来“补救”**——这类代码会被权限门卫直接拒绝，即使没被拒绝也不能作为数字报告。\n\n## 不要做的事\n- 不要自己写分割、计数或形态测量代码（cv2 / skimage / cellpose / YOLO 直接调用都不行）；这是硬性规则。
- 不要在没有像素尺寸时给出任何微米数字。
