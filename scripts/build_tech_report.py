...
e5 = J(f"{R}/e5_agent_tasks_claude_v2.json"); e5v1 = J(f"{R}/e5_agent_tasks_claude.json")["summary"]; P = J(f"{D}/pipeline_demo_data.json"); C = J(f"{D}/cellpose_demo_data.json")
OVB = J(f"{S}/overlay_bold.json"); QCB = J(f"{S}/qc_bold.json")
E4 = J(f"{R}/e4_qc_eval.json"); TM = J(f"{R}/timing.json")
ZH = dict(intestine="intestine", colon="colon", pdac="pdac")
e4rows = "".join(f"<tr><td>{o}</td><td class='n'>{v['n_instances']}</td><td class='n'>{v['n_low']}</td><td class='n'>{v['n_bad']}</td><td class='n'>{v['iou_mean_low']:.2f}</td><td class='n'>{v['iou_mean_high']:.2f}</td><td class='n'>{v['bad_rate_low']*100:.0f}%</td><td class='n'>{v['bad_rate_high']*100:.0f}%</td><td class='n'>{v['flag_precision']:.2f} / {v['flag_recall']:.2f}</td><td class='n'>{v['spearman']:.2f}</td></tr>" for o, v in E4.items())
_lo = [v['bad_rate_low'] for v in E4.values()]; _hi = [v['bad_rate_high'] for v in E4.values()]
e4text = (f"三种器官上，被标为低可信的实例确实明显更差：它们里错误实例的占比是其余实例的 {min(a/b for a,b in zip(_lo,_hi)):.1f} 到 {max(a/b for a,b in zip(_lo,_hi)):.1f} 倍，平均 IoU 也低得多，说明翻转旋转一致性是一个有效的可靠性信号。"
          f"但召回率不高（{min(v['flag_recall'] for v in E4.values()):.2f} 到 {max(v['flag_recall'] for v in E4.values()):.2f}）：多数错误实例在翻转旋转后仍然稳定地被错误分割，尤其是 pdac 上大量的小实例漏检本来就不会以实例形式出现，质控标记无法捕捉“漏掉的”，只能捕捉“不稳定的”。因此报告里把它定位为提示而不是过滤：帮助用户把复核精力放在最可疑的实例上，而不是替用户决定删掉谁。")
_cases = TM["cases"]; _keys = sorted(_cases, key=lambda k: int(k.replace("px", "")))
timing_head = "".join(f"<th class='n'>{k}（{_cases[k]['n_instances']} 个实例）</th>" for k in _keys)
_rows = [("分割（专用权重 + 逐图尺寸估计）", "segment_s"), ("质控：翻转旋转一致性（4 次）", "qc_tta4_s"), ("质控：图像级指标", "image_qc_s"), ("形态测量 + 汇总", "measure_s"), ("叠加图", "overlay_s"), ("检测计数（YOLO11m）", "count_yolo_s")]
timing_rows = "".join(f"<tr><td>{n}</td>" + "".join(f"<td class='n'>{_cases[k][key]:.2f} s</td>" for k in _keys) + "</tr>" for n, key in _rows) + "<tr><td>显存峰值</td>" + "".join(f"<td class='n'>{_cases[k]['gpu_peak_gb']:.1f} GB</td>" for k in _keys) + "</tr>"
timing_note = f"只用 CPU 时，{TM['cpu_segment_s']['size']} 图的分割耗时 {TM['cpu_segment_s']['seconds']:.0f} 秒，仍可用于小批量。报告生成（含直方图与缩略图）另需约 2 到 5 秒。"
def ap(tag):
...
<div class="meta"><b>参赛类别</b><span>端到端系统（End-to-End System）</span><b>团队</b><span>Orgalyst Lab：Xiaoyan Li（AI / 计算机视觉 / 大模型应用，项目负责人）；许力文、付旭正（生物学，类器官培养与形态学判读，负责需求与结果审核）；姜翠翠（AI，大模型、Agent 架构与编排）；杨茹美（生物学，类器官分析）。团队构成：AI / 计算机两人，生物学三人，符合赛事对跨学科团队的要求</span><b>代码</b><span>https://github.com/xiaoyanLi629/orgalyst（MIT）</span><b>权重</b><span>https://huggingface.co/XiaoyanLi/orgalyst-weights</span><b>数据</b><span>OrgLine（Zenodo 16355179，CC-BY-4.0），未使用任何私有数据</span></div>
</header>
...
<p class="small">脑类器官每张图恰好一个类器官（240 张测试图来自 4 个克隆 × 3 个个体 × 10 个时间点 × 2 个实验室），因此它同时是生长曲线和"与专家标注比面积"实验的天然材料；pdac 的测试图每张有 500 个以上的小实例，训练图平均只有 68 个，这一域内分布偏移是第 7、8 节讨论的主要失败来源；结肠只有 15 张训练图，验证集没有掩码。</p>
...
<figure><img src="{shrink(f'{A}/architecture.png', 1600, 86)}"><figcaption><b>图 1a · 系统架构。</b>用 archify 从代码仓库生成，每个节点都对应一个源码文件（节点右上角的 SRC 标记）。左侧虚线框是可选的助手层：界面、助手、分析技能与权限闸门；右侧虚线框是不依赖大模型的确定性工具包：命令行、分割器、检测器、形态计量与质控、统计与生长曲线、报告生成器，以及权重与运行目录。助手只能通过 MCP 服务器的六个工具进入工具包，命令行与助手走同一条代码路径，所以两种用法得到完全相同的数字。交互版（可点击节点查看上下游与源码）见 docs/orgalyst.architecture.html。</figcaption></figure>
<figure><img src="{shrink(f'{A}/overview_board.png', 1400, 80)}"><figcaption><b>图 1b · 一张肠类器官照片的完整旅程。</b>白卡片是数据，绿盒子是模型或工具，大卡片是关键环节。从"一句话"到助手选工具，再到量大小、缩放、切块、U-Net（装着肠专用权重）、三张输出图、聚成轮廓、146 个类器官轮廓、测量表、质检、统计、报告生成器，最后是报告加运行记录。交互版见 docs/orgalyst_scene.html。</figcaption></figure>
...
<p><b>开源模型路径。</b>Claude Agent SDK 支持通过 <code>ANTHROPIC_BASE_URL</code> 指向任何兼容 Anthropic Messages API 的网关，我们在 agent/README.md 里给出了 LiteLLM 前置 vLLM 的配置方式，E5 的任务集脚本可以对不同后端各跑一遍。受时间所限，本报告只给出 Claude 后端的结果，这一点在第 8 节作为局限如实列出。</p>
...
<h2 id="s5">5 作为一个分析助手：能问什么，怎么做</h2>
<p>把前面的工具包和助手放在一起看，Orgalyst 对使用者呈现的是一个会做类器官图像分析的助手：实验人员用中文或英文描述要做的事，助手按标准流程调用工具、核对结果、用固定格式汇报，并把报告和数据留在本次对话的产物目录里。它不是聊天机器人加几个函数，而是把一套分析规程装进了助手，下面从"能问什么"、"按什么流程做"、"界面长什么样"三个方面说明。</p>
<h3>5.1 能问什么</h3>
<div class="tw"><table><tr><th>用户会怎么问</th><th>助手做什么</th><th>用到的工具</th><th>用户得到什么</th></tr>
<tr><td>"这批图里有几个类器官？"</td><td>用检测模型逐图计数，阈值按器官取标定值</td><td><code>count_organoids</code></td><td>逐图数量表、画了框的图、误差范围说明</td></tr>
<tr><td>"分析这批肠类器官照片，告诉我大小和形状"</td><td>按器官加载专用权重分割，测量每个实例，生成报告</td><td><code>analyze_images</code></td><td>总数、逐图表、面积 / 直径 / 圆度的中位数与四分位范围、report.html、features.csv</td></tr>
<tr><td>"像素尺寸是 3.16 µm，面积换成平方微米"</td><td>把标定传给工具，所有长度与面积以微米报告</td><td><code>analyze_images(pixel_size_um=…)</code></td><td>微米单位的结果；没给标定时明确说"只报像素"，绝不估算</td></tr>
<tr><td>"哪些分割结果靠不住？图片质量有问题吗？"</td><td>开启质控：图像级指标 + 翻转旋转一致性</td><td><code>analyze_images(qc=True)</code></td><td>清晰度 / 光照 / 饱和指标，橙色标出的低可信实例及其数量</td></tr>
<tr><td>"对照组和处理组的面积有没有差异？"</td><td>按分组文件做非参数检验与效应量</td><td><code>compare_groups</code></td><td>各组中位数、p 值、Cliff's delta、箱线图、compare.html，结论说人话</td></tr>
<tr><td>"从第 2 天到第 30 天长了多少？"</td><td>按个体与时间点连成生长曲线</td><td><code>growth_curves</code></td><td>每个个体首末时间点的数值与倍数、按组的均值曲线、growth.html</td></tr>
<tr><td>"这批图有肠也有脑，分别分析"</td><td>拆成两批，各用对应器官的权重</td><td>两次 <code>analyze_images</code></td><td>两个运行目录，各自的数量与形态汇总</td></tr>
<tr><td>"圆度是怎么算的？为什么排除贴边的？"</td><td>解释指标定义与规则，不重新计算</td><td>（无）</td><td>两三句话的解释</td></tr>
<tr><td>"这次用了什么模型、参数？以后怎么复现？"</td><td>读运行清单</td><td><code>run_summary</code> / <code>list_runs</code></td><td>manifest.json 的路径及其记录的输入哈希、权重哈希、参数、版本</td></tr>
<tr><td>"分析 /data/x 里的图"（目录是空的 / 器官没说）</td><td>说明没有找到图片；或用通用模型并声明精度较低</td><td>—</td><td>一句明确的说明，而不是一个看起来正常的数字</td></tr></table></div>
<p class="small">表 5-1 · 助手覆盖的问题类型。前十行正是 E5 任务集的十二个任务所覆盖的范围（7.5 节）。</p>
<h3>5.2 按什么流程做</h3>
...
<p class="small">表 6-1 · 各阶段耗时（scripts/bench_timing.py，results/timing.json；RTX 5090 D，肠专用权重 + sizemodel）。{timing_note}</p>
...
<h2 id="s7">7 实验与结果</h2>
<h3>7.1 E1 分割：零样本、按器官微调、直径策略与跨器官泛化</h3>
...
<figure><img src="{img(f'{A}/gallery.jpg')}"><figcaption><b>图 7-1 · 系统默认流程在测试集上的示例（12 张，未挑选：每种器官取前几张）。</b>脑三张为不同克隆、不同天数、两个实验室的图，专用权重加两遍推理都准确框出了整个类器官；肠、胰腺癌、结肠为多实例图，彩色填充为预测实例，标题给出预测数、人工数和该图的 AP50；最后两张为肺类器官的检测计数，橙框为检测框。可以看到典型的误差来源：肠第二张的小碎片被漏掉（AP50 0.61），胰腺癌第一张预测多于人工（224 对 192），结肠的贴边与粘连实例。</figcaption></figure>
...
<h3>7.4 E4 质控的作用</h3>
<p>质控模块的价值在于把错误"标出来"而不是"消掉"。它有没有标对？我们在三种有真值掩码的器官测试集上做了检验：对每个预测实例算它与任一真值实例的最大 IoU，IoU 低于 0.5 记为"错误实例"（多余、拆碎或严重偏移），再看翻转旋转一致性低于 0.5 的"低可信"标记与错误实例的对应关系。</p>
<div class="tw"><table><tr><th>器官</th><th class="n">预测实例</th><th class="n">标为低可信</th><th class="n">错误实例（IoU<0.5）</th><th class="n">低可信组平均 IoU</th><th class="n">其余实例平均 IoU</th><th class="n">低可信中错误占比</th><th class="n">其余中错误占比</th><th class="n">标记精确率 / 召回率</th><th class="n">Spearman ρ</th></tr>{e4rows}</table></div>
<p class="small">表 7-3 · E4 质控标记 vs 真实分割质量（scripts/e4_qc_eval.py，results/e4_qc_eval.json）。精确率 = 被标为低可信的实例里确实错误的比例；召回率 = 错误实例里被标出来的比例；ρ 为一致性与 IoU 的秩相关。</p>
<p>{e4text}</p>
<p>在肠类器官示例图上，120 个实例的翻转旋转一致性平均 0.98，只有 1 个被标为低可信，它正是图像角落一个被截断的团块；在 E5 的质控任务里，4 张肠图 205 个实例中有 3 个被标出。</p>

<h3>7.5 E5 Agent 端到端任务集</h3>
...
<p>表 4 是在第 5 节的 skill 和第 8 节所述的代码拦截都加入之后的完整复跑（<code>results/e5_agent_tasks_claude_v2.json</code>）。加入之前的第一轮同样 12/12 通过，但平均每任务 {e5v1['mean_seconds']:.0f} 秒、{e5v1['mean_tool_calls']:.1f} 次工具调用、合计 {e5v1['total_cost_usd']:.2f} 美元（<code>results/e5_agent_tasks_claude.json</code>）；有了流程约束后耗时和费用都降了约一成到两成，Orgalyst 工具的调用次数反而从 1.5 次升到 1.9 次，因为助手在工具失败时改为尝试工具提供的其他策略而不是自己写代码。值得注意的是工具调用的分布：多数任务只需要一到三次调用，助手拿到工具返回的汇总就直接作答；组间比较和生长曲线两个任务分别用了 31 和 46 次调用、5 到 8 分钟，因为助手在拿到统计结果后自行读取了比较表和生长表逐项核对，又用 Python 重算了一遍中位数。这种"多此一举"提高了可信度但增加了时间和费用，在生产环境里可以通过系统提示约束。器官未知的任务里助手没有反问，而是直接用通用模型跑并在回答里注明精度可能较低，这符合我们的约定。</p>
...
<p><b>大模型相关风险。</b>助手可能选错器官、编造像素尺寸、或者在结果不确定时给出过于肯定的措辞。我们的缓解是结构性的：所有数值来自工具，工具返回里带着单位与排除说明，系统提示禁止在未知像素尺寸时估算，权限门卫限制文件访问；E5 任务集就是对这些约束的回归测试。但 12 个任务不能覆盖真实使用里的提问方式，我们只在 Claude 一个后端上评测，开源模型后端的差距没有量化。此外助手依赖付费 API，评审复现 Agent 路径需要自己的密钥；命令行路径不受此限制，报告里的每个数字都可以由它复现。准备附录 A 的演示时我们还观察到一个反例：在一组第 30 天脑类器官的组间比较里，脑模型漏掉了 TH2-7 组三张图中的两张（正是 E3 里漏检的那几张），助手把这个失败放在回答最前面如实说明，这是我们希望看到的；但它接着自己写了一段阈值分割代码“补救”出这两张图的面积并用于检验，这违反了 skill 里“不自己写分割代码”的约定。助手的诚实和越界在同一个回答里出现，说明只靠提示词约束是不够的。我们随后在权限门卫里加入了硬性拦截（<code>agent/bioagent/permissions.py</code> 的 CODE_GUARDS：Bash、REPL、写文件里出现 cv2 / skimage 阈值分割、直接调用 Cellpose 或 YOLO、regionprops 等图像测量调用一律拒绝，并提示改用 Orgalyst 工具、失败时如实报告），skill 里也加了对应的规则，然后完整复跑了 E5（7.5 节）。同一个第 30 天组间比较任务里，助手这次如实列出两张失败图、只尝试了工具提供的另两种直径策略和通用模型（都无效）、明确写明“这个 p 值没有任何解释价值”，并给出重拍和补标注的建议，没有再自写代码。附录里的组间比较演示用的是第 16 天的照片，两张生长曲线演示截图是在拦截加入之前录的。</p>
