...
<p>助手收到请求后先用任务清单工具把工作拆成三到八步并告诉用户打算怎么做，然后逐步调用工具。它需要做的判断其实很少：这批图是什么器官（用户没说就用 generic 并提醒）、有没有像素尺寸（没有就只报像素，绝不编造）、用户要的是"数数"还是"量大小"、要不要质控、分组和时间点信息在哪个文件里。每次工具调用都落在当前对话的产物目录里，回答时引用 run_dir 和报告路径。权限门卫（<code>agent/bioagent/permissions.py</code>）保证原始数据目录只读、其他账号的目录和密钥目录禁止访问，用户可以为每个对话选择确认方式（逐条确认、只确认危险命令、全部自动、只读）。</p>
...
<p>12 个任务全部通过。值得注意的是工具调用的分布：多数任务只需要一到三次调用，助手拿到工具返回的汇总就直接作答；组间比较和生长曲线两个任务分别用了 31 和 46 次调用、5 到 8 分钟，因为助手在拿到统计结果后自行读取了比较表和生长表逐项核对，又用 Python 重算了一遍中位数。这种"多此一举"提高了可信度但增加了时间和费用，在生产环境里可以通过系统提示约束。器官未知的任务里助手没有反问，而是直接用通用模型跑并在回答里注明精度可能较低，这符合我们的约定。</p>
